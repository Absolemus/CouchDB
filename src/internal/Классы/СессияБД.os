...
Перем КодыПодтверждения; // Массив кодов подтверждения
#КонецОбласти
...
Процедура СоздатьБазуДанных(ИмяБазыДанных) Экспорт
	...
	Если КодыПодтверждения.Найти(Ответ.КодСостояния) = Неопределено Тогда
...
Процедура СоздатьВложение(ИмяВложения, ИмяДокумента, ИмяБазыДанных, СтруктураДанных) Экспорт
	...
	Если СтруктураДанных.ТипДанных = тип("Строка") Тогда
		Заголовки.Вставить(Заголовок, "text/plain");
	ИначеЕсли СтруктураДанных.ТипДанных = Тип("ДвоичныеДанные") Тогда
		Заголовки.Вставить(Заголовок, "application/octet-stream");
	Иначе
		Заголовки.Вставить(Заголовок, "application/json");
	КонецЕсли;
...
Функция БазаДанныхСуществует(ИмяБазыДанных)
	...
	// TO DO
	// Избавиться от магического числа
	Существует = КодыПодтверждения.Найти(Ответ.КодСостояния) <> Неопределено;
...
КодыПодтверждения = Новый Массив;
КодыПодтверждения.Добавить(200);
КодыПодтверждения.Добавить(201);
#КонецОбласти